...
window.onload=function(){
    document.getElementById("pagination").addEventListener("click", changeClass);
    appendTable(parseInt(1))
}


function changeClass(event){ 
    if (event.target.text == "Forward"  ){
        id = document.querySelector("a[class='active']").id
        console.log(id)
        next_id = (parseInt(id) + 1) % 5
        if (next_id == 0) {
            next_id = 5
        }
        document.querySelector("a[class='active']").className = ""
        document.getElementById(next_id.toString()).className = "active";
        appendTable(next_id)
        ...
        appendTable(parseInt(event.target.text))
    }
...
function appendTable(id){
    final_string = ""
    data.map((object) => {
        if ((id*20 - 20 + 1) <= object.id && object.id <= id*20){
            final_string  = final_string + `<tr><td>${object.id}</td><td>${object.name}</td><td>${object.email}</td></tr>`
        }  
    })
